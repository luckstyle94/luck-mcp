...
WHERE d.project = \$1 AND d.path LIKE \$3 AND COALESCE\(d.tags, ARRAY\[\]::text\[\]\) @> \$4::text\[\] AND d.kind = \$5
...
LIMIT \$6`
	...
	mock.ExpectQuery(q).
		WithArgs("luck-mpc", "[0.11,0.22,0.33]", "internal/%", sqlmock.AnyArg(), "summary", 8).
		WillReturnRows(rows)
